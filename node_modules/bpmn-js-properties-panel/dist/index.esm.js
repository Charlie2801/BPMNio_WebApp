...
import { reduce, isArray, find, forEach, sortBy, without, isUndefined, has, findIndex, isDefined, filter, map, keys, flatten, groupBy, isObject, isString, isNil, values, isFunction, assign } from 'min-dash';
import { PropertiesPanel, DebounceInputModule, usePrevious, SelectEntry, isCheckboxEntryEdited, isSelectEntryEdited, CheckboxEntry, isTextAreaEntryEdited, TextAreaEntry, isTextFieldEntryEdited, TextFieldEntry, Group, FeelEntry as FeelEntry$1, FeelTextAreaEntry as FeelTextAreaEntry$1, isFeelEntryEdited, isToggleSwitchEntryEdited, ToggleSwitchEntry, ListGroup, ListEntry, SimpleEntry, CollapsibleEntry, useLayoutState, HeaderButton, ArrowIcon, CreateIcon, DropdownButton } from '@bpmn-io/properties-panel';
...
function isMessageSupported$1(element) {
  return is(element, 'bpmn:ReceiveTask') || isAny(element, ['bpmn:StartEvent', 'bpmn:EndEvent', 'bpmn:IntermediateThrowEvent', 'bpmn:BoundaryEvent', 'bpmn:IntermediateCatchEvent']) && !!getMessageEventDefinition$1(element);
}
...
function isMessageEndEvent(element) {
  return is(element, 'bpmn:EndEvent') && !!getMessageEventDefinition$1(element);
}
function isMessageThrowEvent(element) {
  return is(element, 'bpmn:IntermediateThrowEvent') && !!getMessageEventDefinition$1(element);
}
...
function VariableEventProps(props) {
  ...
  if (!is(element, 'bpmn:StartEvent') || isInEventSubProcess$1(element)) {
    entries.push({
...
let ElementTemplates$1 = class ElementTemplates {
  ...
  _getTemplateVerions(id, options = {}) {
    ...
    const getVersions = template => {
      ...
      return latestOnly ? !includeDeprecated && latest.deprecated ? [] : [latest] : values(versions);
    };
    if (isNil(id)) {
