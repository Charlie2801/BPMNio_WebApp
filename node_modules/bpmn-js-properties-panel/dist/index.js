...
function isMessageSupported$1(element) {
  return ModelUtil.is(element, 'bpmn:ReceiveTask') || ModelingUtil.isAny(element, ['bpmn:StartEvent', 'bpmn:EndEvent', 'bpmn:IntermediateThrowEvent', 'bpmn:BoundaryEvent', 'bpmn:IntermediateCatchEvent']) && !!getMessageEventDefinition$1(element);
}
...
const EMPTY_OPTION$5 = '';
const CREATE_NEW_OPTION$5 = 'create-new';
...
function ErrorRef$1(props) {
  ...
  const getOptions = () => {
    let options = [{
      value: EMPTY_OPTION$5,
      label: translate('<none>')
    }, {
      value: CREATE_NEW_OPTION$5,
 ...
 * @returns {Array<Entry>} entries
 */
function MessageProps$2(props) {
  const {
    element
  } = props;
  if (!isMessageSupported$1(element)) {
    return [];
  }
...
const FeelTextAreaEntry = withTooltipContainer(propertiesPanel.FeelTextAreaEntry);
...
function isZeebeServiceTask(element) {
  ...
  if (ModelUtil.is(element, 'bpmn:EndEvent') || ModelUtil.is(element, 'bpmn:IntermediateThrowEvent')) {
    return !!getMessageEventDefinition$1(element);
  }
...
function isMessageEndEvent(element) {
  return ModelUtil.is(element, 'bpmn:EndEvent') && !!getMessageEventDefinition$1(element);
}
function isMessageThrowEvent(element) {
  return ModelUtil.is(element, 'bpmn:IntermediateThrowEvent') && !!getMessageEventDefinition$1(element);
}
...
function isTimerDefinitionTypeSupported(timerDefinitionType, element) {
  ...
  switch (timerDefinitionType) {
    case 'timeDate':
      ...
      return false;
    case 'timeCycle':
      if (ModelUtil.is(element, 'bpmn:StartEvent') && !isInterruptingStartEvent(businessObject)) {
        return true;
      }
...
function getLegacyId(event) {
  if (ModelUtil.is(event, 'bpmn:IntermediateCatchEvent') || isInterruptingBoundaryEvent(event)) {
    return 'timerEventDefinitionDurationValue';
  }
}
function isInterruptingBoundaryEvent(event) {
  const bo = ModelUtil.getBusinessObject(event);
  return ModelUtil.is(bo, 'bpmn:BoundaryEvent') && bo.get('cancelActivity') !== false;
}
...
function updateErrorGroup$1(groups, element) {
  const errorGroup = findGroup$2(groups, 'error');
  if (!errorGroup) {
    return;
  }
...
function updateEscalationGroup$1(groups, element) {
  const escalationGroup = findGroup$2(groups, 'escalation');
  if (!escalationGroup) {
    return;
  }
...
function updateMessageGroup$1(groups, element) {
  const messageGroup = findGroup$2(groups, 'message');
  if (!messageGroup) {
    return;
  }
...
function updateTimerGroup$1(groups, element, injector) {
  const timerEventGroup = findGroup$2(groups, 'timer');
  if (!timerEventGroup) {
    return;
  }
...
function updateMultiInstanceGroup$1(groups, element) {
  const multiInstanceGroup = findGroup$2(groups, 'multiInstance');
...
function findGroup$2(groups, id) {
...
function VariableEventProps(props) {
  ...
  if (!ModelUtil.is(element, 'bpmn:StartEvent') || isInEventSubProcess$1(element)) {
    entries.push({
...
function ProcessVariablesEntry(props) {
  ...
  hooks.useEffect(async () => {
    ...
    const withName = populateElementNames(sortByName$1(rawVariables));
...
const EXTENSION_BINDING_TYPES$1 = [MESSAGE_ZEEBE_SUBSCRIPTION_PROPERTY_TYPE, ZEBBE_INPUT_TYPE, ZEEBE_OUTPUT_TYPE, ZEEBE_PROPERTY_TYPE, ZEEBE_TASK_DEFINITION_TYPE_TYPE, ZEEBE_TASK_HEADER_TYPE];
  ...
  if (TASK_DEFINITION_TYPES.includes(type)) {
    ...
    commands.push({
      ...
      context: {
        ...
        moddleElement: extensionElements,
        properties: {
          values: minDash.without(extensionElements.get('values'), oldTaskDefinition)
        }
      }
    });
  }
    ...
    if (!properties.length) {
      commands.push({
        ...
        context: {
          ...
          moddleElement: extensionElements,
          properties: {
            values: minDash.without(extensionElements.get('values'), zeebeProperties)
          }
        }
      });
    } else {
...
let ElementTemplates$1 = class ElementTemplates {
  ...
  set(templates) {
    ...
    templates.forEach(template => {
      ...
      if (minDash.isUndefined(latestVerions) || template.version > latestVerions) {
  ...
  _getTemplateVerions(id, options = {}) {
    ...
    const getVersions = template => {
      ...
      return latestOnly ? !includeDeprecated && latest.deprecated ? [] : [latest] : minDash.values(versions);
    };
    if (minDash.isNil(id)) {
      return minDash.flatten(minDash.values(templates).map(getVersions));
    }
...
class Validator extends Validator$1 {
  ...
  _validateTemplate(template) {
    ...
    const validationResult = elementTemplatesValidator.validateZeebe(template);
    const {
      errors,
      valid
    } = validationResult;
...
function canHaveReferencedElement(element) {
  return ModelUtil.isAny(element, ['bpmn:ReceiveTask', 'bpmn:SendTask', 'bpmn:Event']);
}
  ...
  constructor(eventBus, elementTemplates, modeling, commandStack, bpmnFactory) {
    ...
    this.preExecute('shape.create', context => {
      ...
      generatedProps.forEach(p => {
        if (!getPropertyValue$1(element, p)) {
          return;
        }
...
exports.CloudElementTemplatesValidator = Validator;
